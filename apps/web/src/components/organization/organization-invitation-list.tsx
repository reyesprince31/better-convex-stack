...
export function OrganizationInvitationList({
  ...
  return (
    <section className="border border-border/70 bg-background" aria-labelledby="pending-invitations-title">
      ...
      {isLoading ? (
        ...
        <div>
          {pendingInvitations.map((invitation) => (
            <div key={invitation.id} className="flex items-start justify-between gap-3 border-b border-border/70 px-5 py-4 last:border-b-0">
              <div className="min-w-0">
                ...
                <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  {formatOrganizationRole(invitation.role)} · expires {new Date(invitation.expiresAt).toLocaleDateString()}
                </p>
              </div>
              {canCancel ? (
                <Button
        ...
        <div className="p-8 text-center">
          ...
          <p className="mt-3 text-sm font-medium">No pending invitations</p>
          <p className="mt-1 text-xs text-muted-foreground">New invitations will appear here until they are accepted.</p>
        </div>
      )}
    </section>
  );
