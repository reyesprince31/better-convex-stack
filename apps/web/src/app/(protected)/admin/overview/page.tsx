  ...
  return (
    <div className="space-y-10">
      <section className="flex flex-col justify-between gap-5 border-b border-border/70 pb-8 sm:flex-row sm:items-end">
        <div>
          <p className="font-mono text-[10px] tracking-[0.18em] text-muted-foreground uppercase">Administration / overview</p>
          <h1 className="mt-3 text-4xl font-medium tracking-[-0.06em]">Admin console</h1>
          <p className="mt-3 text-sm text-muted-foreground">Manage access, organizations, and the health of the workspace.</p>
        </div>
      ...
      <div className="border border-emerald-500/25 bg-emerald-500/5 p-5 text-sm">
        <div className="flex items-center gap-2 font-medium"><ShieldCheck className="size-4 text-emerald-500" /> Admin access confirmed</div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {[
          ...
          <div key={label} className="border border-border/70 bg-background p-5">
            ...
            <p className="mt-2 font-mono text-[10px] tracking-[0.12em] text-muted-foreground uppercase">{label}</p>
          </div>
        ))}
      </section>
    </div>
