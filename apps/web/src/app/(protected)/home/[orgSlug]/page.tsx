    ...
    <div className="space-y-10">
      <section className="flex flex-col justify-between gap-5 border-b border-border/70 pb-8 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-medium tracking-[-0.06em] sm:text-5xl">{organizationName}</h1>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {[
          { label: "Active projects", value: "12", note: "4 in delivery" },
          { label: "Team members", value: "18", note: "Across 3 circles" },
          { label: "This week", value: "86%", note: "Momentum score" },
        ].map((stat) => (
      ...
      <section className="grid gap-8 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="border border-border/70 bg-background">
          ...
          <div className="divide-y divide-border/70">
            {["Spring launch", "Customer interviews", "New onboarding flow"].map((project, index) => (
              <div key={project} className="flex items-center gap-4 px-5 py-5">
                <span className="font-mono text-xs text-muted-foreground">0{index + 1}</span>
                <div className="flex-1"><p className="text-sm font-medium">{project}</p><p className="mt-1 text-xs text-muted-foreground">Updated {index + 1}h ago</p></div>
                <span className="flex items-center gap-1.5 text-xs text-muted-foreground"><Check className="size-3.5 text-emerald-500" /> On track</span>
              </div>
            ))}
          </div>
        </div>
        <div className="border border-border/70 bg-background p-5">
          ...
          <p className="mt-8 text-4xl font-medium tracking-[-0.07em]">18</p>
          <p className="mt-2 text-xs text-muted-foreground">People creating momentum together.</p>
          <div className="mt-8 flex -space-x-2">
            {["AL", "MK", "JS", "RN", "+14"].map((initials, index) => <span key={initials} className={`flex size-8 items-center justify-center rounded-full border-2 border-background text-[10px] font-medium ${index === 4 ? "bg-muted text-muted-foreground" : "bg-foreground text-background"}`}>{initials}</span>)}
          </div>
        </div>
      </section>
    </div>
