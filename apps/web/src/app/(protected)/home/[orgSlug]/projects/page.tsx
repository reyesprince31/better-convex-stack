...
export default function OrganizationProjectsPage({
  ...
  return (
    <Suspense fallback={<ProjectsPageLoading />}>
      <OrganizationProjectsContent params={params} />
    </Suspense>
  );
}

async function OrganizationProjectsContent({
  params,
}: {
  params: Promise<{ orgSlug: string }>;
}) {
  ...
  return (
    <OrganizationResourcePage
      ...
      title="Projects"
      description="A focused view of the work your team is moving forward."
      icon={FolderKanban}
    >
        ...
        {organization.projects.map((project) => (
          <div key={project.id} className="grid gap-4 px-5 py-5 sm:grid-cols-[1fr_auto] sm:items-center">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">{project.team} / due {project.due}</p>
            </div>
            <div className="flex items-center gap-3 sm:min-w-48">
              <div className="h-1 flex-1 bg-muted">
                <div className={`h-1 ${project.color}`} style={{ width: `${project.progress}%` }} />
              </div>
...
function ProjectsPageLoading() {
  return <div className="h-96 animate-pulse bg-muted" aria-label="Loading projects" />;
}
