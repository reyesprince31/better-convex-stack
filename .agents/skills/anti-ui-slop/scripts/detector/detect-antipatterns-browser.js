/**
 * Anti-Pattern Browser Detector for Uizze
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__UIZZE_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip uizze's own elements
    if (el.closest && el.closest('.uizze-overlay, .uizze-label, .uizze-banner, .uizze-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.uizzeExtension === 'true')
    || document.documentElement.dataset.uizzeExtension === 'true';
  ...
  styleEl.textContent = `
    @keyframes uizze-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .uizze-overlay:not(.uizze-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: uizze-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .uizze-overlay.uizze-visible {
      animation-play-state: running;
    }
    .uizze-overlay.uizze-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .uizze-overlay.uizze-hover .uizze-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .uizze-overlay.uizze-spotlight {
      z-index: 100002 !important;
    }
    .uizze-overlay.uizze-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .uizze-spotlight-backdrop {
      position: fixed;
    ...
    .uizze-spotlight-backdrop.uizze-visible {
      opacity: 1;
    }
    .uizze-hidden .uizze-overlay${EXTENSION_MODE ? '' : ':not(.uizze-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'uizze-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__UIZZE_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('uizze-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('uizze-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('uizze-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._uizzeOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('uizze-visible');
  ...
  function detachOverlay(overlay) {
    ...
    if (overlay._targetEl && overlay._targetEl._uizzeOverlay === overlay) {
      visibilityObserver.unobserve(overlay._targetEl);
      delete overlay._targetEl._uizzeOverlay;
    }
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('uizze-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('uizze-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('uizze-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'uizze-overlay uizze-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('uizze-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('uizze-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.uizze-overlay, .uizze-label, .uizze-banner, .uizze-tooltip')) continue;
      if (el.closest('[id^="uizze-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.uizze-overlay, .uizze-label, .uizze-banner, .uizze-tooltip')) continue;
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[uizze] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[uizze] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__UIZZE_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip uizze's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.uizze-overlay, .uizze-label, .uizze-banner, .uizze-tooltip')) continue;
      // Skip browser extension elements (Claude, etc.)
      const elId = el.id || '';
      if (elId.startsWith('claude-') || elId.startsWith('cic-')) continue;
      // Skip the uizze live-mode overlay (highlight, tooltip, bar, picker, toast).
    ...
    // Clone the document and strip uizze-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="uizze-live-"]')) {
      node.remove();
    }
  ...
  function shouldRunVisualContrast(options = {}) {
    return options.visualContrast === true || window.__UIZZE_CONFIG__?.visualContrast === true;
  }

  function visualContrastOptions(options = {}) {
    const config = window.__UIZZE_CONFIG__ || {};
  ...
  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('uizze-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[uizze] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('uizze-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'uizze-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__UIZZE_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('uizze-hidden');
        document.body.classList.toggle('uizze-hidden', visible);
        window.postMessage({ source: 'uizze-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('uizze-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('uizze-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('uizze-spotlight', isMatch);
              o.classList.toggle('uizze-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('uizze-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('uizze-spotlight');
          o.classList.remove('uizze-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'uizze-ready' }, '*');
  } else {
    if (window.__UIZZE_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[uizze] scan failed', err);
        }
      };
  ...
  window.uizzeDetect = detect;
  window.uizzeDetectAsync = detectAsync;
  window.uizzeScan = scan;
  window.uizzeScanAsync = scanAsync;
  window.uizzeCollectVisualContrastCandidates = collectVisualContrastCandidates;
  window.uizzeAnalyzeVisualContrast = analyzeVisualContrast;
  window.uizzeGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
}
