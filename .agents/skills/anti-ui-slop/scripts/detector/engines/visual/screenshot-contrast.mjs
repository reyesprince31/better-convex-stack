...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `uizze-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(
    ({ selector, token, backgroundClipText }) => {
      ...
      if (!el) return false;
      let style = document.getElementById("uizze-visual-contrast-hide-style");
      if (!style) {
        style = document.createElement("style");
        style.id = "uizze-visual-contrast-hide-style";
        style.textContent = [
          "[data-uizze-visual-contrast-target] {",
          "  color: transparent !important;",
          "  -webkit-text-fill-color: transparent !important;",
          "  text-shadow: none !important;",
          "}",
          '[data-uizze-visual-contrast-target][data-uizze-bgclip-text="true"] {',
          "  background-image: none !important;",
          "}",
        ].join("\n");
        document.head.appendChild(style);
      }
      el.setAttribute("data-uizze-visual-contrast-target", token);
      if (backgroundClipText) el.setAttribute("data-uizze-bgclip-text", "true");
      return true;
    },
    {
      ...
      .evaluate(
        ({ selector }) => {
          try {
            ...
            if (el) {
              el.removeAttribute("data-uizze-visual-contrast-target");
              el.removeAttribute("data-uizze-bgclip-text");
            }
          } catch {
