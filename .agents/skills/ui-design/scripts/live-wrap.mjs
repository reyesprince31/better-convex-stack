 ...
 *
 * Usage:
 *   node scripts/live-wrap.mjs --id SESSION_ID --count N --query "hero-combined-left" [--file path]
 *
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx ? [
    indent + '<div data-uizze-variants="' + id + '" data-uizze-variant-count="' + count + '" ' + styleContents + '>',
...
function buildCssSelectorPrefixExamples(styleMode, count) {
  ...
  return Array.from({ length: count }, (_, i) => `[data-uizze-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === 'astro-global-prefixed') {
    return {
      ...
      selectorExamples: variantNumbers.map((n) => `[data-uizze-variant="${n}"] > .variant-class`),
      requirements: [
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: 'scope-rule',
    rulePattern: '@scope ([data-uizze-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map((n) => `@scope ([data-uizze-variant="${n}"]) { :scope > .variant-class { ... } }`),
    requirements: [
