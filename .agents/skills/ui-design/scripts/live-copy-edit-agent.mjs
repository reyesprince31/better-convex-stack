...
export async function runCopyEditBatchAgent(batch, opts = {}) {
  ...
  const outDir = opts.outDir || fs.mkdtempSync(path.join(os.tmpdir(), "uizze-copy-batch-"));
...
export function runCopyEditPostApplyChecks({ cwd = process.cwd(), files = [] } = {}) {
  ...
  for (const relativeFile of uniqueFiles) {
    ...
    if (markerMatch)
      failures.push({ file: relativeFile, reason: "leftover_uizze_marker", marker: markerMatch });
...
function findLeftoverUizzeMarker(content) {
  ...
  const attrPattern = /\bdata-uizze-(?:variants?|original-text|editable|text-wrap)\s*=/g;
...
export function describeNoProviderError({
  ...
  lines.push("Fix one of the above, or set UIZZE_LIVE_COPY_AGENT=mock for tests.");
  return lines.join("\n");
}
