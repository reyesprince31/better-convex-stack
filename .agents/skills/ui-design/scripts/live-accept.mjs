...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  if (cssContent) {
    replacement.push(
      indent + commentSyntax.open + " uizze-carbonize-start " + id + " " + commentSyntax.close,
    );
    ...
    replacement.push(indent + '<style data-uizze-css="' + id + '">' + (isJsx ? "{`" : ""));
    ...
    replacement.push(
      indent + commentSyntax.open + " uizze-carbonize-end " + id + " " + commentSyntax.close,
    );
  }

  // Keep the `@scope ([data-uizze-variant="N"])` selectors in the
  // carbonize CSS block working visually by re-wrapping the accepted content
  // in a data-uizze-variant="N" div with `display: contents` (so layout
  ...
  if (cssContent) {
    ...
    replacement.push(indent + '<div data-uizze-variant="' + variantNum + '" ' + styleAttr + ">");
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp("uizze-variants-end\\s+" + escapeRegExp(id) + "(?:\\s|--|\\*/|$)").test(line);
}
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-uizze-variant="' + variantNum + '"');
...
function findSessionFile(id, cwd) {
  const marker = "uizze-variants-start " + id;
