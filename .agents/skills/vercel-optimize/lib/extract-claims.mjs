...
function recommendsSharedCache(rec) {
  const haystack = [rec?.what, rec?.why, rec?.fix, rec?.desiredBehavior, rec?.verify]
    .filter(Boolean)
    .join("\n");
...
function mentionsRuntimeCacheWhenCacheComponents(rec) {
  ...
  const citations = asArray(rec?.citations).join("\n");
  return (
    /\b(?:Runtime Cache|@vercel\/functions|getCache\s*\(|setCache\s*\()\b/i.test(haystack) ||
    /vercel\.com\/docs\/caching\/runtime-cache/i.test(citations)
  );
}
...
function mentionsAuthSensitiveParallelization(rec) {
  const haystack = recText(rec);
  if (!/\b(?:parallelize|Promise\.all|run concurrently|start .* early)\b/i.test(haystack))
    return false;
  if (
...
function mentionsCachedNotFoundOr404(rec) {
  const haystack = recText(rec);
  if (!/\b(?:s-maxage|CDN-Cache-Control|Vercel-CDN-Cache-Control|Cache-Control)\b/i.test(haystack))
    return false;
...
function mentionsTurboBuildCacheRecommendation(rec) {
  const haystack = recText(rec);
  if (!/\b(?:Turbo|Turborepo|turbo\.json|tasks\.build|build cache|build caching)\b/i.test(haystack))
    return false;
  return (
