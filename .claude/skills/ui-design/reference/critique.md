...
- **AI slop**: Would someone believe "AI made this" immediately? Check all DON'T guidance from the parent Uizze skill.
...
2. **Pass the structured metadata** through `UIZZE_CRITIQUE_META` (JSON), then run the write command:
   ...
   ```bash
   UIZZE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
