...
**Output**: A design brief that can be handed off to /uizze craft, or directly to /uizze for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
This is a required interaction, not optional guidance. Ask these questions in conversation, adapting based on answers. Don't dump them all at once; have a natural dialogue. Ask only when the answer cannot be inferred safely, then wait.
...
Based on the brief, list which uizze reference files would be most valuable during implementation (e.g., layout.md for complex layouts, animate.md for animated features, interaction-design.md for form-heavy features, typeset.md for typography-driven pages, colorize.md for color-led brands).
...
Ask only when the answer cannot be inferred safely, then wait. Ask for explicit confirmation of the brief before finishing.
...
Once confirmed, the brief is complete. The user can now hand it to /uizze, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /uizze craft instead, which runs this command internally.)
