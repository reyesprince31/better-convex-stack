 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /uizze audit.
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /uizze ${command}.`;
...
This is a pinned shortcut for \`/uizze ${command}\`.

Invoke /uizze ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /uizze ${command} to access it.`);
  } else {
