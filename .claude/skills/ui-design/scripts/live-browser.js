...
(function () {
  ...
  const PICKER_SHADOW = "0 0 0 1px oklch(78% 0.12 82 / 0.18), 0 10px 28px oklch(0% 0 0 / 0.28)";
  ...
  const sessionState = window.__UIZZE_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, "").slice(0, 8),
  });
  if (!sessionState) {
    console.error("[uizze] live-browser-session.js was not loaded. Live mode cannot start safely.");
  ...
  const ICONS = {
    uizze: `<svg ${ICON_ATTRS}><path d="M4 20l4-1L18 9l-3-3L5 16z"/><path d="M14 7l3 3"/></svg>`,
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.("[data-uizze-placeholder-resize]")?.dataset
      .uizzePlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    for (const node of nodes) {
      const runtimeEditable =
        node.hasAttribute("data-uizze-editable") || node.hasAttribute("data-uizze-original-text");
  ...
  function resolveBarAnchor() {
    if (currentSessionId && (state === "GENERATING" || state === "CYCLING")) {
      const wrapper = document.querySelector('[data-uizze-variants="' + currentSessionId + '"]');
      if (wrapper) {
  ...
  function buildConfigureRow() {
    ...
    input.type = "text";
    input.placeholder =
      selectedAction === "uizze" ? "describe what you want…" : "refine further (optional)…";
    ...
    if (!document.getElementById(PREFIX + "-configure-input-style")) {
      ...
      s.textContent =
        ...
        "#" +
        PREFIX +
        '-configure-voice[data-listening="true"] svg { animation: uizze-configure-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-uizze-variant="' + visibleVariant + '"]');
  }
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf("uizze-") !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || "").slice(0, 500),
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = "@keyframes uizze-spin { to { transform: rotate(360deg); } }";
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.uizzeTextWrap !== "true" || !el.parentElement) return null;
  ...
  function showVariantInDOM(sessionId, num) {
    const wrapper = document.querySelector('[data-uizze-variants="' + sessionId + '"]');
    if (!wrapper) return;
    for (const child of wrapper.children) {
      const v = child.dataset ? child.dataset.uizzeVariant : null;
      if (!v) continue;
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      ...
      .then((html) => {
        ...
        srcWrapper = doc.querySelector('[data-uizze-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.error("[uizze] Variant wrapper not found in source file.");
          return;
        }
        ...
        const existingWrapper = document.querySelector('[data-uizze-variants="' + sessionId + '"]');
        if (existingWrapper) {
          ...
          if (!liveEl) {
            console.error("[uizze] Could not find original element in live DOM.");
            return;
          }
        ...
        expectedVariants = parseInt(wrapper.dataset.uizzeVariantCount || arrivedVariants);
        ...
        positionBar();
        saveSession();
        console.log("[uizze] Injected " + arrivedVariants + " variants from source file.");
      })
      .catch((err) => {
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-uizze-variant="' + index + '"]');
  ...
  function startScrollTracking() {
    function tick() {
      if (state === "CONFIGURING" || state === "GENERATING" || state === "CYCLING") {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.("data-uizze-insert-placeholder")) {
  ...
  function connectSSE() {
      ...
      switch (msg.type) {
        case "connected":
          ...
          if (!hasProjectContext)
            showToast(
              "No PRODUCT.md found. Variants will be brand-agnostic. Run /uizze init to generate one.",
              7000,
            );
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log(
          "[uizze] SSE connection lost. Retry " + sseRetries + "/" + SSE_MAX_RETRIES + "...",
        );
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn("[uizze] capture failed, proceeding without screenshot:", err);
    }
...
void main() {
  ...
  function handleAccept() {
    ...
    function scheduleAcceptCleanup() {
      ...
      // on or the app isn't React at all. Preserve the `data-uizze-variant="N"`
      ...
      setTimeout(function () {
        const wrapper = document.querySelector('[data-uizze-variants="' + acceptedSessionId + '"]');
        if (!wrapper) return;
        const accepted = wrapper.querySelector('[data-uizze-variant="' + acceptedVariant + '"]');
  ...
  function cleanup() {
    ...
    if (cleanupSessionId) {
      const wrapper = document.querySelector('[data-uizze-variants="' + cleanupSessionId + '"]');
    ...
    setTimeout(function () {
      ...
      const wrapper = document.querySelector('[data-uizze-variants="' + cleanupSessionId + '"]');
      if (!wrapper) return;
      const orig = wrapper.querySelector('[data-uizze-variant="original"]');
      if (orig) {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(
        el("span", {
          ...
          animation: "uizze-steer-dot 1.05s ease-in-out " + i * 0.14 + "s infinite",
        }),
      );
    }
    return wrap;
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute(
      "aria-label",
      connected ? "Uizze live mode" : "Uizze live mode — agent not polling",
    );
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      if (!detectScriptLoaded) {
        ...
        window.postMessage({ source: "uizze-command", action: "scan" }, "*");
      } else {
        detectPendingScan = true;
      }
    } else {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>/uizze document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .uizze/design.json.</strong> Run <code>/uizze document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>/uizze document</code> also generates a <code>.uizze/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log("[uizze] Live variant mode ready. Hover over elements to pick one.");
      ...
      const scout = new MutationObserver(() => {
        ...
        if (resumeSession()) {
          console.log(
            "[uizze] Resumed deferred session " + currentSessionId + " (post-hydration).",
          );
        }
      });
